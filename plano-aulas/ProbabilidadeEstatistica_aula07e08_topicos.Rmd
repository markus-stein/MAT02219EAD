...
**Axiomas de Kolmogorov**: seja $A$ um evento definido no espaço amostral $\Omega$ de um experimento $E$, então a medida (número real) $P(A)$ é denominada a probabilidade de ocorrência do evento $A$ se  
...
* Eventos condicionados: probabilidade de ocorrer $A$ dado que $B$ ocorreu, $P(\left. A \right| B)$;
    + eventos independentes $P(\left. A \right| B) = P(A) \times P(B)$.

## Teorema da Probailidade Total e Teorema de Bayes (seção 5.4, Livro Bussab e Morettin)
* Partição do espaço amostral: seja $B_1, B_2, B_3, \ldots, B_k$ (para $k \in \mathbb{N}$) uma partição do espaço amostral, então
...
Seja $A$ um evento definido no espaço amostral $\Omega$ associado ao experimento $E$ e $B_1, B_2, B_3, \ldots, B_k$ uma partição de $\Omega$, então
...
### Teorema de Bayes**
*Também é possível calcular a probabilidade de ocorrência de uma partição $B_i$ dados que um evento $A$ ocorreu.*
